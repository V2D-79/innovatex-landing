...
export type LimelightNavProps = {
  ...
  activeIndex?: number; // Added to support controlled state via scroll-spy
...
export const LimelightNav = ({
  ...
  const [activeIndex, setActiveIndex] = useState(externalActiveIndex ?? defaultActiveIndex);
  const [isReady, setIsReady] = useState(false);
  const navItemRefs = useRef<(HTMLAnchorElement | null)[]>([]);
  const limelightRef = useRef<HTMLDivElement | null>(null);

  // Sync with external active index if provided
  useEffect(() => {
  ...
  return (
    <nav className={`relative inline-flex items-center h-16 rounded-lg bg-transparent text-foreground px-2 ${className}`}>
